...
def book(new_data=x, filename='data.json'):
    ...
    if a == True:
        if json.load(open(filename, 'r+'))["book_details"][0]['book_status'] == False:
            with open(filename, 'r+') as file:
                file_data = json.load(file)
                file_data["book_details"][0]['book_status'] = True
                file.seek(0)
...
def cancell_show(new_data=x, filename='data.json'):
    ...
    if a == True:   
        if json.load(open(filename, 'r+'))["book_details"][0]['book_status'] == True:
            with open(filename, 'r+') as file:
                file_data = json.load(file)
                file_data["book_details"][0]['book_status'] = False
                file.seek(0)
                json.dump(file_data, file, indent=4)
                import mod_data
                del mod_data
...
mov1 = Label(films_list, image=mov1x).place(x=0, y=0)
...
cancell = Button(root, text="Cancel if aldready booked", padx=10, pady=10, command=cancell_show).place(x=325, y=750)
